...
class TemplateLibrarySourceSpec extends Specification{
    ...
    def setupSpec(){
        StepWrapper.metaClass.static.createFromString = { String file, CpsScript s, String stepName, String libName, Map config ->
            return new StepWrapper(name: stepName)
        }
    }
    def setup(){
    ...
    def "loadLibrary loads each groovy file in library as a step"(){
        ...
        then: 
            binding.hasVariable("a")
            binding.getVariable("a") instanceof StepWrapper
            binding.hasVariable("b")
            binding.getVariable("b") instanceof StepWrapper
            !binding.hasVariable("c")
    }
